                 ...
                 tags$hr(),
                  ...
                  tabPanel("Requerimientos de almacenamiento",headerPanel('Requirements'),
                           ...
                           sidebarPanel( HTML("<h3>Input parameters</h3>"),
                                         ...
                                         tags$hr(), 
                                         numericInput('Demanda', 'Recepcion Promedio', 1000, min = 1, max = 20000),
                                         tags$hr(), 
                                         numericInput('Picking', 'Picking Promedio', 3000, min = 1, max = 20000),
                                         tags$hr(), 
                                         numericInput('Devolucion', 'Devolucion Promedio', 3000, min = 1, max = 20000),
                                         tags$hr(),
                           ...
                           #fluidRow(column(4),tags$label(h3('Output'),column(4),actionButton("submitbutton105", "Submit", class = "btn btn-primary"))),
                           #verbatimTextOutput("value"),
                           reactableOutput("values2"),
                           fluidPage(fluidRow(column(2, uiOutput("demo_ui")),
                           column(2, uiOutput("demo_ui2")),column(2, uiOutput("demo_ui3")) ))
                           )),
...
server <- function(input, output) {
  ...
  output$demo_ui2 <- renderUI({
    #cols <- names(iris)[1:4]
    #selectInput("y", "Column to plot", cols, "Sepal.Length")
    numericInput("obs2", "Observations:", 10,
                 min = 1, max = 100)
  })
  output$demo_ui3 <- renderUI({
    #cols <- names(iris)[1:4]
    #selectInput("y", "Column to plot", cols, "Sepal.Length")
    numericInput("obs2", "Observations:", 10,
                 min = 1, max = 100)
